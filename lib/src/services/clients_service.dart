...
class ClientsService extends ChangeNotifier {
  ...
  Future<List<ClientModel>> loadClients() async {
    ...
    final url = Uri.https(_baseUrl, '/api/users'); //Enviamos el cuerpo y los parametros
  ...
  void updateImagesPreview(String imageType, String path) async {
    ...
    if(fileSizeInMegabytes < 5.0){
      ...
      NotificationsService.showSnackbar('No se puede subir un archivo superior a 5MB', 'error');
    }
    notifyListeners();
  }
  ...
  Future<int> createClient (ClientModel client) async {
    ...
    final fileIne = await http.MultipartFile.fromPath('ine', client.ine);
    packageUpload.files.add(fileIne);
    final fileIneBack = await http.MultipartFile.fromPath('ine_back', client.ineBack);
    packageUpload.files.add(fileIneBack);
    final filePayStub = await http.MultipartFile.fromPath('pay_stub', client.payStub);
    packageUpload.files.add(filePayStub);
    final fileSelfie = await http.MultipartFile.fromPath('selfie', client.selfie);
    packageUpload.files.add(fileSelfie);
    final fileProofAddress = await http.MultipartFile.fromPath('proof_address', client.proofAddress);
    packageUpload.files.add(fileProofAddress);
